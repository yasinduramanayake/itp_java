...
<input type="search" name="searchhousep" id="myInput5" placeholder="Search.." ><input type="submit" value="search"></form></div></li>
<li>
...
<center><h1 style = "font-family: Arial, Helvetica, sans-serif;
				        color:white;
				        font-size: 2.0em;
				        text-shadow: 3px 2px #black;">Furniture and wooden panelling/partitoning</h1></center>
				        <center>
	<div class="container" style="background-color:#f1f1f1;background-color: rgba(255,255,255,0.5); width:50% ;height:30%;">			        
...
String  sql1= "select * from pastproject where Type=?";

preparedStatement =connection.prepareStatement(sql1);


preparedStatement.setString(1,"FURNITURE AND WOODEN PANELLING / PARTITONING");
...
<td><form  method="post" action="preupdated_deigning_projects">
			
	           
					    <input type="text"  name="Type" id="Type"  value="<%=res.getString("Type")%>" required hidden>
            <input type="number"  step="1"  name="proect_ID" id="Type"  value="<%=res.getString("proect_ID")%>" required hidden>
  
				      
						<input name="myprojects"  type="text" id="itemImageB641" value="<%=res.getString("image")%>" hidden >
						
						<input type="text"  hidden name="Description"  id ="Description" value="<%=res.getString("Description")%>"  required >
			             <input type="number"  step="1" hidden name="linkproject"   value="8"  required >
			...
			<form method="post" action="PastProjectDelete">
			
			<input type="text"  name="proect_ID" id="Type" type ="text" value="<%=res.getString("proect_ID")%>" required hidden>
			<input type="number"  step="1" hidden name="linkproject" type ="text"  value="8"  required >
...
<script type="text/javascript">
		document.getElementById('itemImage1').onchange = function() {
			var file = document.getElementById('itemImage1').files[0];
			var reader = new FileReader();
			reader.readAsDataURL(file);
			reader.onload = function() {
				//console.log(reader.result);
				document.getElementById('itemImageB641').value = reader.result;
			};
